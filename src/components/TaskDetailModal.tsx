    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(26,27,46,0.55)] p-4" onClick={onClose}>
      <div className="card-base w-full max-w-3xl p-5" onClick={(event) => event.stopPropagation()}>
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-lg font-extrabold text-text">{draft.title}</h2>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-[1.5fr_1fr]">
          ...
          <div className="space-y-3">
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.06em] text-text-dim">Trạng thái</label>
              <select
                value={draft.status}
