...
export default function PeoplePage() {
  ...
  if (loading) return <LoadingState title="Đang tải thành viên" />;

  return (
    <div className="min-h-screen bg-bg">
      <ViewHeader
        title="Thành viên"
        subtitle={`${people.length} thành viên trong workspace`}
        right={<button onClick={() => setOpenDM(true)} className="btn-base rounded-xl bg-primary px-3 py-2 text-xs font-semibold text-white">DM mới</button>}
      />
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {people.map((user) => (
            <article key={user.id} className="card-base p-4">
              ...
              <p className="text-xs text-text-muted">{user.status_text || "Không có trạng thái"}</p>
              <div className="mt-3 flex gap-2">
                <button className="btn-base rounded-lg border border-border bg-bg-light px-2.5 py-1.5 text-xs font-semibold text-text">Nhắn tin</button>
                <button className="btn-base rounded-lg border border-border bg-bg-light px-2.5 py-1.5 text-xs font-semibold text-text">Hồ sơ</button>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(26,27,46,0.55)] p-4" onClick={() => setOpenDM(false)}>
          <div className="card-base w-full max-w-lg p-5" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-base font-extrabold text-text">Bắt đầu DM mới</h2>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setOpenDM(false)} className="btn-base rounded-lg border border-border px-3 py-2 text-sm">Hủy</button>
              <button onClick={() => setOpenDM(false)} className="btn-base rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-white">Bắt đầu chat ({picked.length})</button>
            </div>
          </div>
        </div>
      ) : null}
